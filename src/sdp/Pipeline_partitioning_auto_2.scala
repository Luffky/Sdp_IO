...
object Pipeline_partitioning_auto_2 {
  ...
  def stringHash(key: String): Int = {
    val digest = MessageDigest.getInstance("MD5")
    val temp = Integer.valueOf(digest.digest(key.getBytes).map("%02x".format(_)).mkString.substring(0, 7), 16)
    temp
  }
  ...
  def main(args: Array[String]) {
   ...
   // System.setProperty("spark.serializer", "org.apache.spark.serializer.KryoRegistrator")
    ...
    val conf = new SparkConf().setAppName("SDP Pipeline").setMaster("local[1]")
    ...
    class MapPartitioner[T](partitions: HashMap[T, Int]) extends Partitioner {
      override def getPartition(key: Any):
        Int = partitions(key.asInstanceOf[T])
    ...
    val extract_lsm: RDD[((Int, Int), Data)] = {
      println("the ************* scale_data    "+scale_data)
...
//   broads_input.unpersist()
   ...
   //   val input_sum_facets: RDD[((Int, Int, Int, Int), Data)] =
    ...
    val subimacom: RDD[((Int, Int, Int, Int), Data)] = {
      ...
      input_identify_component.partitionBy(partitioner).cogroup(input_sum_facets.partitionBy(partitioner)).mapPartitions(subimacom_kernel2)
    }
    // === Update LSM ===
    val update_lsm: RDD[((Int, Int), Data)] = {
      ...
      input_local_sky_model.partitionBy(partitioner).cogroup(input_source_find.partitionBy(partitioner)).mapPartitions(update_lsm_kernel2)
    }
    // === Terminate ===
    println("Finishing...")
    ...
    println("\n\n--------------------------------")
    println("past:[" + (end - start)/1000000000.0 + "s]")
    println("--------------------------------")
  }
 ...
 /*def identify_component_kernel : (Iterator[((Int,Int,Int,Int), Data)]) => Iterator[((Int,Int,Int,Int), Data)] = { case (data_sum_facets) =>
  ...
  def subimacom_kernel2: Iterator[((Int, Int, Int, Int), (Iterable[Data], Iterable[Data]))] => Iterator[((Int, Int, Int, Int), Data)] = {
    case ite =>
      val temp = ite.next()
      var ix = temp._1
      val (data_identify_component, data_sum_facets) = temp._2
      var hash: Int = 0
      var input_size: Long = 0
      ix = (0, 0, 0, 0)
      val label: String = "Subtract Image Component (73224.4 MB, 67.14 Tflop) " + ix.toString
  ...
  def update_lsm_kernel2: Iterator[((Int, Int), (Iterable[Data], Iterable[Data]))] => Iterator[((Int, Int), Data)] = {
    case ite =>
      var hash: Int = 0
      var input_size: Long = 0
      var ix: (Int, Int) = (0, 0)
      for (temp <- ite){
        ix = temp._1
        val (data_local_sky_mode, data_source_find) = temp._2
        for (data <- data_local_sky_mode){
          hash ^= data(0)
          input_size += data.length
        }
